...
export default function PassengerDetails() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('agentBooking');
    if (!stored) {
  ...
  const validateForm = () => {
    ...
    if (!/^[0-9]{10}$/.test(contactDetails.mobile)) {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      // Navigate to payment
      navigate(`/agent/payment?bookingId=${bookingRef.id}`);
          ...
          <div className="bg-white rounded-lg shadow-md p-6 h-fit sticky top-6">
            ...
            <div className="border-t pt-4 space-y-2 mb-6">
              ...
              <div className="flex justify-between text-green-600">
                ...
                <span className="font-bold">₹{bookingData.commission.toFixed(0)}</span>
              </div>
            </div>
